...
Next step is to load your data. Here, we used the d_example_guide.rds data available from the GitHub repository. 
It is simulated from football data for the example to be reproducible.
The object "d" in the r-code below can be replaced with your own data. 
At the minimum, your data should have:
<ul>
 <li>one column for load, measured in any metric (such as sRPE, GPS measures or ACWR)</li>
 <li>one column for injury, must either be a logical variable (TRUE/FALSE) or coded (0 for no injury, 1 for injury) to 
     work in our logistic regression model.</li>
 <li> one column for athlete ID, coupling the load values and injuries to the right person</li>
</ul>
...
according to the documentation, knots are placed based on data quartiles.
...
`predictInterval()` can't be used on anything but a `glmer()`-created object from the `lme4`-package.
